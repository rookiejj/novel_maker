...
### 5. Keep-Alive (Free Tier 대응)

Supabase Free Tier는 **7일 연속 DB activity가 없으면 프로젝트가 자동 pause**됩니다 (URL 접속은 activity로 집계되지 않고 실제 DB 쿼리가 필요). 90일간 paused 상태가 지속되면 복구가 불가능하므로, `.github/workflows/supabase-keepalive.yml`이 매주 월·목 03:17 UTC (KST 12:17)에 `mood_records` 테이블에 SELECT 쿼리를 날려 activity를 유지합니다. RLS 때문에 결과는 빈 배열이지만 쿼리 자체는 실행되어 activity로 인정됩니다.

GitHub Secrets에 아래 두 값을 등록해야 동작합니다 (Repo → Settings → Secrets and variables → Actions):

| Secret | 값 |
| --- | --- |
| `SUPABASE_URL` | `https://<your-project-ref>.supabase.co` |
| `SUPABASE_ANON_KEY` | Supabase Dashboard → Settings → API → `anon public` 키 |

Pro 플랜($25/월+)으로 업그레이드하면 비활성 pause가 적용되지 않으므로 이 워크플로우는 불필요해집니다.
