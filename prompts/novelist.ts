...
export function buildSystemPrompt({ config, recentMoods }: PromptInput): string {
  ...
  const narrativeDevice = NARRATIVE_DEVICES[Math.floor(Math.random() * NARRATIVE_DEVICES.length)];

  return `당신은 ${persona}입니다.
하나의 긴 이야기를 여러 편에 걸쳐 이어가는 연재 소설을 씁니다.
...
위 장치를 자연스럽게 녹여서 예측 불가능한 이야기를 만드세요.
