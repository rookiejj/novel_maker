...
function buildProtagonistSection(name?: string, gender?: string, genre?: string): string {
  if (!name && !gender) return '';
  const lines: string[] = [];
  if (name)   lines.push(`- 이름: **${name}** (반드시 이 이름을 사용할 것. 변경·생략 금지)`);
  if (gender) {
    lines.push(`- 성별: **${gender}** (이 성별을 절대 바꾸지 말 것. 대명사·호칭·외형·복장 묘사 모두 일관되게 ${gender}으로 유지. 다른 편에서도 동일.)`);
  }
  const blLine = genre === 'BL'
    ? '\n- **장르 제약:** 이 작품은 BL입니다. 주인공과 로맨스 상대역은 **반드시 모두 성인 남성**입니다. 여성 로맨스 상대·여성 주연 등장 금지. 여성 조연이 등장하더라도 로맨스의 중심이 되어서는 안 됩니다.'
    : '';
  return `\n## 주인공 (엄격히 준수)\n${lines.join('\n')}${blLine}\n`;
}
...
export function buildSystemPrompt({ config, recentMoods }: PromptInput): string {
  ...
  const protagonistSection = buildProtagonistSection(protagonistName, protagonistGender, genre);
